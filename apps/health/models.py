...
class FasilitasHealth(models.Model):
    health = models.ForeignKey(Health, on_delete=models.CASCADE, related_name='fasilitas')
    fasilitas = models.CharField(max_length=255)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Fasilitas for {self.health.title}"

class ImageHealth(models.Model):
    health = models.ForeignKey(Health, on_delete=models.CASCADE, related_name='images')
    image = models.ImageField(upload_to='health/', help_text=_("Image size max 10MB and max resolution 2000x2000px auto compress"))
    ...
    def compress_image(self, image, max_size=2000, quality=88):
        ...
        while True:
            ...
            if quality == 60 and max_size < 800: # px
                break
    ...
    def save(self, *args, **kwargs):
        # Jika alt_text kosong, otomatis isi dengan title health
        if not self.alt_text:
            self.alt_text = self.health.title
        super().save(*args, **kwargs)
